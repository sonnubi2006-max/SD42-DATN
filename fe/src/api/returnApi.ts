...
const uploadReturnImage = async (file: File): Promise<string> => {
  ...
  fd.append("file", file);
  const { data } = await axiosInstance.post<string>("/returns/upload", fd, {
...
export const returnApi = {
  create: async (payload: CreateReturnRequest): Promise<ReturnResponse> => {
    const { data } = await axiosInstance.post<ReturnResponse>("/returns", payload);
    return data;
  },
  ...
  getById: async (id: number): Promise<ReturnResponse> => {
    const { data } = await axiosInstance.get<ReturnResponse>(`/returns/${id}`);
    return data;
  },
  ...
  refundPreview: async (
    ...
    const { data } = await axiosInstance.post<RefundPreviewResponse>("/returns/refund-preview", {
      orderId,
      items,
    });
    return data;
