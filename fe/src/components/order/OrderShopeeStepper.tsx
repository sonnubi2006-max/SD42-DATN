...
export default function OrderShopeeStepper({ order, baseSteps }: OrderShopeeStepperProps) {
  ...
  return (
    <Card className="border border-emerald-100 bg-gradient-to-b from-emerald-50/20 to-white shadow-2xs">
      <CardContent className="py-8 px-4 sm:px-10">
        <div className="relative flex items-center justify-between w-full">
          ...
          {(() => {
            ...
            const truckIcon = baseSteps.find(s => s.status === "SHIPPING")?.icon;
            const checkIcon = baseSteps.find(s => s.status === "COMPLETED")?.icon;

            baseSteps.forEach(s => {
              if (initialStatuses.includes(s.status) || s.status === "WAITING_PAYMENT") {
                stepsToRender.push(s);
              }
            });

            const deliveryLogs = order.transactionLogs?.filter(t =>
              ["SHIPPING", "FAILED_DELIVERY", "CANCELED_BY_DAMAGED", "COMPLETED"].includes(t.currentStatus)
            ) || [];
            ...
            } else if (lastAdded !== "COMPLETED" && !["CANCELLED", "REFUNDED", "RETURNED_TO_SHOP", "CANCELED_BY_DAMAGED", "RETURNING"].includes(order.orderStatus)) {
              stepsToRender.push({
                label: "Hoàn thành",
                status: "COMPLETED" as OrderStatus,
                icon: checkIcon,
            ...
            if (order.orderStatus === "CANCELLED" || order.transactionLogs?.some(t => t.currentStatus === "CANCELLED")) {
              stepsToRender.push({ label: "Đã hủy", status: "CANCELLED" as OrderStatus, icon: ShieldAlert });
            }
            if (order.orderStatus === "REFUNDED" || order.transactionLogs?.some(t => t.currentStatus === "REFUNDED")) {
              stepsToRender.push({ label: "Hoàn tiền", status: "REFUNDED" as OrderStatus, icon: ShieldAlert });
            }

            let currentIdx = -1;
            for (let i = stepsToRender.length - 1; i >= 0; i--) {
              const stepStatus = stepsToRender[i].status;
              const isMatch = stepStatus === order.orderStatus ||
                ((order.orderStatus === "RETURNING" || order.orderStatus === "RETURNED_TO_SHOP") && stepStatus === "SHIPPING");
              if (isMatch) {
                currentIdx = i;
                break;
              }
            ...
            const isFinalStatus = ["COMPLETED", "CANCELLED", "REFUNDED", "RETURNED_TO_SHOP", "FAILED_DELIVERY", "CANCELED_BY_DAMAGED", "RETURNING"].includes(order.orderStatus);

            const activeSteps = stepsToRender.filter((step, idx) => {
              // Nếu đơn đã hoàn thành/hủy/hoàn tiền (trạng thái cuối), không hiển thị các bước phía sau
              if (isFinalStatus && idx > currentIdx) {
                return false;
              }
              // Nếu là bước tương lai của đơn đang chạy, hiển thị dạng chờ (upcoming)
              if (idx > currentIdx) {
                return true;
              }
              // Đối với bước hiện tại hoặc quá khứ:
              if (idx === currentIdx) {
                return true;
              }
              if (step.status === "PENDING" || step.status === "WAITING_STOCK" || step.status === "WAITING_PAYMENT" || step.status === "DRAFT" || initialStatuses.includes(step.status)) {
                return true;
              }
            ...
            for (let i = activeSteps.length - 1; i >= 0; i--) {
              if (activeSteps[i].status === order.orderStatus) {
                currentActiveIdx = i;
                break;
              }
            }
