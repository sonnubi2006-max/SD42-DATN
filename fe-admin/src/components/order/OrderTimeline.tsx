import { Calendar, FileText, ArrowRight, Images } from "lucide-react";
                    ...
                    {log.images && log.images.length > 0 && (
                      <div className="mt-2 space-y-1.5">
                        <p className="flex items-center gap-1 text-[10px] font-semibold text-gray-500">
                          <Images size={11} /> Ảnh xác nhận ({log.images.length})
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {log.images.map((image, imageIndex) => (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="block overflow-hidden rounded-md border border-gray-200 bg-gray-50 transition hover:border-blue-400 hover:shadow-sm"
                              title={image.originalName || `Ảnh xác nhận ${imageIndex + 1}`}
                            >
                              <img
                                src={image.imageUrl}
                                alt={image.originalName || `Ảnh xác nhận ${imageIndex + 1}`}
                                loading="lazy"
