...
import type { ProductVariantStatus } from "@/api/productVariantApi";
...
interface ProductVariantToolbarProps {
  ...
  onStatusChange: (v: ProductVariantStatus | "") => void;
  direction: "asc" | "desc";
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1 max-w-md">
          <Input
            ...
            placeholder="Tìm theo mã, kích cỡ, màu..."
...
export function FilterPanel({
  ...
  const productList = useMemo(
    () =>
      products.map<ItemsSearchable>((i) => ({
        id: i.productId,
        name: i.productName,
      })),
    [products],
    ...
    <div className="rounded-xl border border-gray-200 bg-gray-50/60 p-4">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Select
          value={status || "all"}
          onValueChange={(v) =>
            onStatusChange(v === "all" ? "" : (v as ProductVariantStatus))
          }
        >
        ...
        {!id &&
          <SearchableSelect
            items={productList}
            value={productId}
            onChangeSelect={onProductChange}
            search=""
            title="Sản phẩm"
          />
        }
...
export default function ProductVariantToolbar(props: ProductVariantToolbarProps) {
